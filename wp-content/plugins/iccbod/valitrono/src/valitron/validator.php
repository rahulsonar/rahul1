...
class Validator {
    ...
    protected function validateIns($field, $value, $params) {
        $isAssoc = array_values($params[0]) !== $params[0];
        if ($isAssoc) {
            $params[0] = array_keys($params[0]);
        }
    ...
    /**
     * Validate that a field contains only alphabetic characters and space
     *
     * @param  string $field
     * @param  mixed  $value
     * @return bool
     */
    protected function validateAlphas($field, $value) {
        return preg_match('/^([a-z][a-z ]*[a-z])+$/i', $value);
    }
    ...
    /**
     * Validate that a field contains only alpha-numeric characters with spaces
     *
     * @param  string $field
     * @param  mixed  $value
     * @return bool
     */
    protected function validateAlphaNums($field, $value) {
    ...
    public function error($field, $msg, array $params = array()) {
        ...
        if (substr($field, -2) === '.*') {
            ...
            $field = substr($field, 0, $actuallength);
        }
