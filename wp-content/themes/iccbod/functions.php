 ...
 * Custom User Meta
 */
//require (get_template_directory() . '/inc/custom-user-meta.php');
...
function twentytwelve_widgets_init() {
    ...
    register_sidebar(array(
        'name' => __('frontenddashboard', 'twentytwelve'),
        'id' => 'sidebar-4',
        'description' => __('Frontend BOD dashbaord sidebar', 'twentytwelve'),
    ));
}
...
if (!function_exists('twentytwelve_comment')) :
    ...
    //custom menu link login
    add_filter('wp_nav_menu_items', 'add_loginout_link', 10, 2);

    function add_loginout_link($items, $args) {
        if (is_user_logged_in() && $args->theme_location == 'primary') {
            $items .= '<li><a href="' . wp_logout_url() . '" class="login-link lineheeight65">Logout <i class="fa fa-sign-in" aria-hidden="true"></i></a></li>';
        } elseif (!is_user_logged_in() && $args->theme_location == 'primary') {
            $items .= '<li><a href="' . site_url('wp-login.php') . '" class="login-link lineheeight65">Login <i class="fa fa-sign-in" aria-hidden="true"></i></a></li>';
        }
        return $items;
    }

    //Backend Allow Admin Login Only
    ...
    function check_custom_authentication($username) {
        ...
        //Check if email is entered as username
        if (filter_var($username, FILTER_VALIDATE_EMAIL)) {
            ...
            $userObject = get_user_by('email', $username);
        } else {
        ...
        //Clear Auth Cookie and Die for Non-Admin Role users
        if (false === in_array('administrator', $userObject->roles)) {
            wp_clear_auth_cookie();
            $message = "Administrator Login Allowed only <a href=" . site_url() . ">Back to website</a>";
            wp_die($message);
        }
    }
    ...
    function add_theme_footer_scripts() {

        //Stylesheets
        wp_enqueue_style('bootstrapcss', get_template_directory_uri() . '/css/bootstrap.min.css');
        wp_enqueue_style('customcss', get_template_directory_uri() . '/css/custom.css');
        wp_enqueue_style('font-awesomecss', get_template_directory_uri() . '/font-awesome/css/font-awesome.min.css');

        //In footer last args true
        wp_enqueue_script('jquery', get_template_directory_uri() . '/js/jquery.js');
        wp_enqueue_script('iccbodcustomjs', get_template_directory_uri() . '/js/custom.js', array('jquery'), false, true);
        wp_enqueue_script('bootstrapjs', get_template_directory_uri() . '/js/bootstrap.min.js', array('jquery'), false, true);
    }

    add_action('wp_enqueue_scripts', 'add_theme_footer_scripts');

    /*
     * Check User Logged In in Template redirect hook, Redirect Escape Home Page
     */

    function check_login_authentication() {
        if ((get_page_template_slug() != 'page-templates/full-width.php') && (false === is_user_logged_in())) {
            wp_redirect(site_url());
        }
    }

    add_action('template_redirect', 'check_login_authentication');
    
    //Frontent user login
    add_action('wp_userlogin', 'userlogin');
    ...
    function userlogin($username, $password) {
        ...
        if (filter_var($username, FILTER_VALIDATE_EMAIL)) {
            ...
            //Check if email exist in users
            if (!email_exists($username)) {
                $err_messages['username'] = "Username does not exists!!!";
            }
        ...
        if (false === in_array('administrator', $userObject->roles)) {
            ...
            wp_redirect(site_url()."/userprofile");
        }
    }